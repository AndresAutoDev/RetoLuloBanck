...
public class PostRandomUserNoPathValid implements Task {

    private static final Logger LOGGER = LoggerFactory.getLogger(PostRandomUserNoPathValid.class);
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        String endpoint = ENDPOINT_BASE + "/createe";
        ...
        actor.remember("getErroByUserPost", response.getBody().jsonPath().getString("error"));

        LOGGER.debug("La respuesta completa de la tarea ObtenerUsuarios es: {}", response.getBody().prettyPrint());

    }

    public static PostRandomUserNoPathValid conDatosAleatorios() {
        return instrumented(PostRandomUserNoPathValid.class);
    }
