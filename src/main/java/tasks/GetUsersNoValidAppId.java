...
public class GetUsersNoValidAppId implements Task {
    private static final Logger LOGGER = LoggerFactory.getLogger(GetUsersNoValidAppId.class);
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        Response response = RestAssured.given()
                ...
                .header("app-id", "64013f90bb1e4f20bc431c5cs")
                .when()
                .get();
        ...
        actor.remember("errorNoValidAppId", response.getBody().jsonPath().getString("error"));

         LOGGER.debug("La respuesta completa de la tarea ObtenerUsuarios es: {}", response.getBody().prettyPrint());
    }

    public static GetUsersNoValidAppId aleatorios() {
        return instrumented(GetUsersNoValidAppId.class);
    }
}
